...
from util import lrelu, conv2d, maxpool2d, dconv
...
def conv_net(x,  dropout):
    ...
    #convblock 1
    conv1 = slim.conv2d(xs, 4, [256, 1], activation_fn=None, normalizer_fn=slim.batch_norm, scope='conv1')
    print('conv1: ', conv1)
    pool1 = slim.max_pool2d(conv1, [2, 1], scope='pool1')
    print('pool1: ', pool1)

    #convblock 2
    conv2 = dconv(pool1, 16, [128, 1], scope='conv2')
    print('conv2: ', conv2)
    pool2 = slim.max_pool2d(conv2, [2, 1], scope='pool2')
    print('pool2: ', pool2)

    #convblock 3
    conv3 = dconv(pool2, 12, [128, 1], scope='conv3')
    print('conv3: ', conv3)
    ...
    conv4 = tf.reshape(d2s4, shape=[-1, sr, 1, 8])
    conv4 = dconv(conv4, 16, [128, 1], scope='conv4')
    conv4 = lrelu(conv4)
    print('conv4: ', conv4)

    #convblock 5
    conv5 = tf.concat([conv4, lrelu(conv1)], 3) # <- unet concat conv1 with conv4 16+4
    print('concat: ', conv5)
