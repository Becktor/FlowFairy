...
from util import lrelu, conv2d, maxpool2d, diconv2d
...
def conv_net(x,  dropout):
    ...
    #convblock 2
    conv2 = diconv2d(xs, [1, 128, 1, 16], 2, scope='conv2')
    print('conv2: ', conv2)
    pool2 = slim.max_pool2d(conv2, [2, 1], scope='pool2')
    print('pool2: ', pool2)

    #convblock 3
    conv3 = diconv2d(pool2, 4, 2, scope='conv3')
    print('conv3: ', conv3)

    #convblock 4
    d2s4 = tf.depth_to_space(conv3, 2) #upconv
    print('depth2space: ', d2s4)
    # reshape upconvolution to have proper shape
    conv4 = tf.reshape(d2s4, shape=[-1, sr, 1, 4])
    conv4 = diconv2d(d2s4, 4, 2, scope='conv4')
    print('conv4: ', conv4)
