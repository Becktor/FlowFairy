...
def conv_net(x,  dropout):
    ...
    #convblock 1
    conv1 = slim.conv2d(xs, 4, [256,1], rate=[2, 1], activation_fn=lrelu,
    ...
    #convblock 2
    conv2 = slim.conv2d(pool1, 16, [128, 1], rate=[3, 1], activation_fn=lrelu, scope='conv2')
    print('conv2: ', conv2)
    pool2 = slim.max_pool2d(conv2, [2, 1], scope='pool2')
    print('pool2: ', pool2)

    #convblock 3
    conv3 = slim.conv2d(pool2, 16, [128, 1], rate=[3, 1], activation_fn=lrelu, scope='conv3')
    print('conv3: ', conv3)
    ...
    conv4 = slim.conv2d(conv4, 16, [128, 1], rate=[4, 1], activation_fn=lrelu, scope='conv4')
    print('conv4: ', conv4)
