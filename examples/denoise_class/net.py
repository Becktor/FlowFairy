...
def conv_net(x,  dropout):
    ...
    #convblock 1
    conv1 = slim.conv2d(xs, 4, [256, 1], activation_fn=lrelu, normalizer_fn=slim.batch_norm, scope='conv1')
    print('conv1: ', conv1)
    pool1 = slim.max_pool2d(conv1, [2, 1], scope='pool1')
    print('pool1: ', pool1)

    #convblock 2
    conv2 = dconv(pool1, 12, [128, 1], scope='conv2')
    print('conv2: ', conv2)
    pool2 = slim.max_pool2d(conv2, [2, 1], scope='pool2')
    print('pool2: ', pool2)

    #convblock 3
    conv3 = dconv(pool2, 16, [128, 1], activation_fn=lrelu, scope='conv3')
    print('conv3: ', conv3)
    ...
    conv4 = dconv(conv4, 16, [128, 1], activation_fn=lrelu, scope='conv4')
    print('conv4: ', conv4)
