...
def conv_net(x,  dropout):
    ...
    #convblock 1
    conv1 = slim.conv2d(xs, 4, [256, 1], scope='conv1_1')
    print('conv1: ', conv1)
    pool1 = slim.max_pool2d(conv1, [2, 1], scope='pool1')
    print('pool1: ', pool1)

    #convblock 2
    conv2 = slim.conv2d(pool1, 16, [128, 1], scope='conv2_1')
    print('conv2: ', conv2)
    pool2 = slim.max_pool2d(conv2, [2, 1], scope='pool2')
    print('pool2: ', pool2)

    #convblock 3
    conv3 = slim.conv2d(pool2, 16, [128, 1], scope='conv3_1')
    print('conv3: ', conv3)
    ...
    conv4 = slim.conv2d(conv1, 16, [128, 1], scope='conv4_1')
    print('conv4: ', conv4)

    #convblock 5
    conv5 = tf.concat([conv4, conv1], 3) # <- unet like concat first with last
    print('concat: ', conv5)
    conv5 = slim.conv2d(conv1, 256, [1, 1], scope='conv5_1')
    print('conv5: ', conv5)
    #out
